...
///
/// \brief The AbstractTargetTracker class
///
    ...
    m_currFrame(1)
{
  ...
  void Process()
  {
    int m_fps = 15; //customized define the fps
    m_currFrame = 1;
  ...
  void Process2()
  {
    static int framesCounter = 1;
    static int64 allTime = 0;

    if (this->image_raw_.empty())
    {
      ROS_WARN("Invalid image input, maybe no data!");
      return;
    }
    //1) obtain input image
    int currFrame = 0;
    currFrame = this->m_currFrame ? 0 : 1;
    this->m_frameInfo[currFrame].m_frame = this->image_raw_;//get image
    cv::cvtColor(this->m_frameInfo[currFrame].m_frame, this->m_frameInfo[currFrame].m_gray, cv::COLOR_BGR2GRAY);

    //2) init tracker
    if (!this->m_isTrackerInitialized)//zhanghm:use gray image to initialize tracker
    {

      this->m_isTrackerInitialized = this->InitTracker(this->m_frameInfo[currFrame].m_gray);
      if (!this->m_isTrackerInitialized)
      {
     ...
     * ********************/
    int64 t1 = cv::getTickCount();
    this->Detection(this->m_frameInfo[currFrame].m_frame, this->m_frameInfo[currFrame].m_gray, this->m_frameInfo[currFrame].m_regions);
    int64 t2 = cv::getTickCount();
    this->m_frameInfo[currFrame].m_dt = t2 - t1;

    this->m_currFrame = this->m_currFrame ? 0 : 1;
    std::cout << "CaptureAndDetect: thisPtr->m_currFrame = " << this->m_currFrame << std::endl;

    //4) tracking
    /* ********************
     *  Tracking
     * ********************/
    int64 t1_track = cv::getTickCount();
    Tracking(m_frameInfo[currFrame].m_frame, m_frameInfo[currFrame].m_gray, m_frameInfo[currFrame].m_regions);

    int64 t2_track = cv::getTickCount();
    //std::cout << "Process: tracking" << std::endl;
